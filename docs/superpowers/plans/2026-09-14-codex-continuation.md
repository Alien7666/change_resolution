...
### 2026-09-18 Task 17 checkpoint

User authorized Task 17 and subsequently requested a progress report. Current progress: Tasks 1–16 complete, Task 17 in progress, 84/87 steps checked. Task 17 documentation (Step 1) and full post-fix automated verification (Step 3) are complete. The current branch remains `codex/configurable-tray`, HEAD `dc82508`; Task 17 application/docs edits remain uncommitted, and the user's four staged `.idea` files are preserved.

Whole-branch review is complete. Its four important findings (model-key uniqueness, legacy prefill, lost scaling restoration on profile replacement, and lost display recovery after uncertain writes) plus the manifest wording issue have been implemented by the Sol worker. Root also verified `SaveAndReplace` persistence ordering and the recovery paths. Tests, repeated app/scaling tests, build/vet/format/tidy checks, display/NVAPI read-only integrations and the final GUI build passed. Real GUI checks verified legacy prefill and isolated save, plus choosing a second monitor then cancelling without creating a config or temporary file. No hardware settings were written; the four-monitor baseline remained unchanged.

Final scoped re-review by `/root/task17_fix_review` was interrupted by the usage limit before a verdict. Resume that review from `task-17-fixes-review.diff`, the existing brief and implementation report in the plan's ignored workspace. Do not repeat the completed whole-branch review. Outstanding steps: finish scoped re-review (Step 2), commit exact Task 17 paths after acceptance (Step 5), and obtain human hardware-write verification (Step 4). Detailed evidence and remaining scenarios are in [the verification record](../verification/2026-09-18-configurable-tray.md).

### 2026-09-18 Task 17 review and commit acceptance

The user first requested 85/87 steps, then authorized completing Task 17. The resumed scoped review delivered a persisted verdict before another usage interruption: Requirements PASS, Code quality PASS WITH MINOR FOLLOW-UPS, no blocking findings. All five fixes were accepted. The coordinator read the complete report and confirmed the frozen reviewed diff still matched the current application source. Minor follow-ups (unused direct model-save seam/stale comments, and direct idle Provider save-failure coverage) are recorded in the verification document; neither is a current runtime defect.

Application fixes are committed as `a649920`; documentation and these progress records are committed separately. Progress is now 86/87 steps, with only Task 17 Step 4 outstanding. The user's staged `.idea` files remain excluded. No push, release or actual display/scaling write has occurred. Resume with the concrete supervised hardware checklist in the verification record; do not rerun completed development or fabricate the remaining human observations.

### 2026-09-19 supervised hardware checkpoint

The user authorized and observed real writes. XV272K V3's 2048×1536 attempt safely refused before writes because its calculated shift would overlap the two upper displays; a Sol assessment confirmed the refusal follows the spec. 2461W 1440×1080 provided the non-native test path. Managed scaling requested Raw 2 and read Raw 5 with visible bars; unmanaged direct scaling also read Raw 5 but the user observed the bars disappear. Restore was faithful to Raw 6, including normal tray exit; stable NVAPI identity survived DISPLAY5 → DISPLAY8. The managed restore cycle's four stages were sampled, and the restarted watcher restored the original mode after its three-second delay. User confirmed close/minimize and tray reopen work.

Terra implemented the concrete wording correction in `6b5005b`: both reminders now say black bars may appear. Root reviewed the diff, repeated full checks, built the GUI and inspected both updated surfaces. A new isolated first-run profile for 2461W was saved without a mode write. Remaining manual work and exact current desktop state are in the verification record; progress stays 86/87. No push/release, and the user's four staged `.idea` files remain untouched.
