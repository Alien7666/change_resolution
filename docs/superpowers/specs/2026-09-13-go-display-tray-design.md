...
`DisplayController` 負責列舉顯示裝置、以硬體 ID 對應目標螢幕、讀取目前模式、讀取整體桌面排列（每台顯示器的模式、座標與是否為主要顯示器）、驗證目標模式，以及以單一交易套用與恢復整體排列。
...
整體排列以單一交易套用：每台顯示器先以 `CDS_NORESET` 暫存，最後以一次 NULL 裝置／NULL DEVMODEW 呼叫提交。任何一步失敗即中止，桌面不會停在套用到一半的狀態。若無法算出安全排列（顯示器會重疊、有顯示器讀取失敗、主要顯示器會被移動），則不做任何變更並回報錯誤。
...
- Win32 邊界以假的 `user32` 驗證呼叫序列：每台顯示器一次 `CDS_NORESET`，最後一次 NULL 提交，且任何呼叫都不帶 `CDS_UPDATEREGISTRY`。
