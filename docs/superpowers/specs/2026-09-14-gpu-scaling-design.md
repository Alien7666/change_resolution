...
- **接續的是 `seen`。** `Session` 多一個 `gameSeen bool`，生命週期與 `managed` 完全相同：`observeGame` 拿到 `result.SeenGame` 為真就把它設為真，`finishRestore` 釋放擁有權時清為假，第一次 `Enable` 之前是假。`startWatcher` 以它為種子建立 tracker，而且只設 `seen` 這一個欄位。
  ...
  **此處與「循環中接續 `seen`」直接衝突，實作時已裁決。** 循環的第 2 步走的就是 `finishRestore`，照這條字面規則，`seen` 會在循環中途被清掉，接續也就不可能發生。裁決：第 2 步之前把 `gameSeen` 捕捉到區域變數，第 4 步的 `startWatcher` 之前再種回去；若第 4 步結束時並未進入管理狀態，則一併清掉。兩端都留有註解。沒有別的讀法能同時滿足這兩句話。
