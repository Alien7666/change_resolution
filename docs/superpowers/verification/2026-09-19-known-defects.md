# 已知缺陷（2026-09-19 起；1 於 bfc688d、2 與 3 於 6c54796、4 於 1f39bf7、5 於 d309767 修復，全部已實機確認）
...
## 缺陷 5 的結論 — **已修復 `d309767`**，待實機確認

控制測試定案。其他條件全相同（同樣要 raw 2、同樣讀回 raw 5、同樣 `Applied == true`），唯一變數是寫入當下螢幕在哪個解析度：

| 寫入時螢幕在 | 黑邊 |
|---|---|
| 原生 1920×1080（舊的管理循環） | **在** |
| 1440×1080（未管理路徑 `writeScaling`） | **消失** |

兩個事實：

**1. 寫入必須落在目標解析度上。** `NV_SCALING_GPU_SCALING_TO_NATIVE` 要求 GPU 把來源放大到面板原生。在原生解析度下沒有來源可以放大，驅動就改記錄長寬比值。舊的 `runScalingCycle` 固定先 `restoreSaved()` 還原成原生才寫入，等於每一次都寫在那個沒有作用的位置。

**2. 讀回值不是畫面。** 讀回 raw 5（長寬比），畫面卻是全螢幕無黑邊。`GetDisplayConfig` 的 scaling 欄位是驅動的紀錄，不是掃描輸出。

### 修法

拿掉整個循環，就地寫入。循環唯一的存在理由是不讓裝置名稱跨過 NVAPI 寫入，而 `1f39bf7` 的 `remapSavedNames` 已經讓改號可復原，那個理由消失了。

附帶好處：少兩次模式切換、畫面不再閃兩下、寫入失敗不再讓使用者失去已套用的模式（舊循環在呼叫 NVAPI 之前就已經把模式交出去了）。

移除：`StateScalingCycle`、`cyclePhase`、`endCycle`、`viewLocked` 的遮罩、UI 的循環分支與成本提示。

### 文字修正

- 「驅動實際套用的是」→「驅動記錄的值是」，並加註不等於畫面。舊說法把成功的寫入報成失敗，害我們查了好幾輪
- 拿掉所有從讀回值推論黑邊的句子（`scalingAspectReminder`、`modeScalingNote`）
- 按鈕旁那句從「請到 NVIDIA 控制台勾選覆寫遊戲和程式所設定的縮放模式」改成「這個設定只在目前這次有效，切換顯示模式後會回到控制台裡的值」。前者管的是遊戲覆寫縮放，對桌面黑邊無效；後者是實測結果（取消勾選再勾選，黑邊必定回來）
...
| 測試 | 處置 |
|---|---|
| 管理中順序、重新解析、重建 saved、還原鈕同路徑、set 失敗 | 改寫成就地寫入的契約 |
| 循環的顯示還原失敗、重新套用失敗、重新套用不確定寫入 | 刪除（該步驟不存在了） |
| watcher 不能在循環中還原、遊戲在循環中結束 | 改寫，新增 `signalOnScalingSet` 在 NVAPI 寫入時同步 |
| `TestRestoreAppliesTheDisplayLayoutBeforeAnyScalingRestore` | 改寫成「縮放還原不碰顯示模式」。R2-at-exit（關閉時先還原顯示再還原縮放）是另一個測試，仍然通過 |

新增 `TestSavedNamesSurviveAScalingSetThroughTheIdentityRemap` — 釘住兩次修復的交互作用：set 改號後 saved 仍保留舊名字，靠重新對應才還原得回去。

### 突變驗證

- 把 `restoreSaved()` 加回寫入之前 → 5 個測試同時失敗，事件序列顯示 `display.apply` 跑在 `scaling.apply` 前面
- `stateForScalingResult` 固定回 `StateNative` → 管理中寫入後的狀態測試失敗

### 實機確認（2026-09-21）

管理中勾選 1440 × 1080、按「設為全螢幕縮放（GPU）」，黑邊消失。就地寫入在管理路徑上得到與先前未管理路徑相同的結果，假說成立，缺陷關閉。
